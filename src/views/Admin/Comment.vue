...
const deleteSelectedComments = () => {
  ...
  ElMessageBox.confirm("确认删除选中的项吗?", "温馨提示", {
    ...
    .then(async () => {
      // 调用后端删除数据的函数
      await deleteCommentByGroupService({ delete_list: selectedItems.value });
      // 清空选中项
      selectedItems.value = [];
      //刷新列表
      await getComments();
...
<template>
  <el-card class="page-container">
    ...
    <!-- 评论列表 -->
    <el-table ref="multipleTableRef" :data="Comments.slice((pageNum - 1) * pageSize, pageNum * pageSize)" style="width: 100%"
